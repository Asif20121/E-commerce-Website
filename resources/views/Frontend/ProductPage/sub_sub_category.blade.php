...
<div class="content_one" style="min-height: 1000px; padding-top: 30px;">
    ...
    <div style="display: flex;"> <a href="{{ url('/')}}" style="text-decoration: none;">@if (session()->get('language')=='bangla')
       ...
       <h1>Home</h1> 
        @endif </a> <h1> /</h1>
        <a href="{{ route('productPage.category', [ 'mainCategory_id'=>$sub_sub_category->category->id, 'mainCategory_name'=>$sub_sub_category->category->category_name_en] )}}" style="text-decoration: none;"><h1>{{ session()->get('language')=='bangla'?$sub_sub_category->category->category_name_bn : $sub_sub_category->category->category_name_en }} </h1> </a>
        <a href="{{ route('productPage.subcategory', ['subCategory_id'=> $sub_sub_category->subcategory->id, 'subCategory_name'=> $sub_sub_category->subcategory->subcategory_name_en] )}}"  style="text-decoration: none;"> <h1>/ {{ session()->get('language')=='bangla'? $sub_sub_category->subcategory->subcategory_name_bn : $sub_sub_category->subcategory->subcategory_name_en }} </h1> </a>
        <h1>/ {{ session()->get('language')=='bangla'? $sub_sub_category->subsubcategory_name_bn : $sub_sub_category->subsubcategory_name_en }} </h1> 
    </div> 
